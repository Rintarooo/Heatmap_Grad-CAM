...
def build_VGG16_and_predict(img_path):
	model = VGG16(weights = 'imagenet',
	              include_top = True)#import VGG16 model
	              
	img = image.load_img(img_path,
                         target_size = (224,224))#import PIL image
                         
	x = image.img_to_array(img)#PIL --> numpy array(224,224,3)
	x = np.expand_dims(x, axis = 0)#(224,224,3) --> (1,224,224,3)
	x = preprocess_input(x)#Normalize each channel
	preds = model.predict(x)#prediction
	return x, model, preds



def make_heatmap(x, model, preds):
	...
	layer_name = 'block5_conv3'#choose last layer of conv
